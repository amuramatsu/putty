...
typedef BOOL (WINAPI* FN_SetCompositionFontA)(HIMC, LPLOGFONTA);
typedef BOOL (WINAPI* FN_SetCompositionWindow)(HIMC, PCOMPOSITIONFORM);
typedef LONG (WINAPI* FN_GetCompositionStringW)(HIMC, DWORD, LPVOID, DWORD);
...
static FN_SetCompositionFontA   fn_SetCompositionFontA   = NULL;
static FN_SetCompositionWindow  fn_SetCompositionWindow  = NULL;
static FN_GetCompositionStringW fn_GetCompositionStringW = NULL;
...
{
    if (fn_SetCompositionFontA)
        return fn_SetCompositionFontA(imc, lf);
    return FALSE;
}
...
LONG WINAPI ImmGetCompositionStringW(HIMC imc, DWORD x,
				     LPVOID buf, DWORD buflen)
{
    if (fn_GetCompositionStringW)
        return fn_GetCompositionStringW(imc, x, buf, buflen);
    return -1;
}
